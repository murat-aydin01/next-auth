...
Bir kullanıcı oluşturun ve kullanıcıya admin rolü atayın.  
Actions > Library > Create Action > Build From Scratch (Trigger: post login)
```
exports.onExecutePostLogin = async (event, api) => {
  const namespace = 'kayraexport/role';
  const roles = event.authorization?.roles[0] || "user";
